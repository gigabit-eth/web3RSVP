const hre = require("hardhat");

const main = async () => {
    // deploy the contract
    const rsvpContractFactory = await hre.ethers.getContractFactory("Web3RSVP");
    const rsvpContract = await rsvpContractFactory.deploy();
    await rsvpContract.deployed();
    console.log("Web3RSVP deployed to:", rsvpContract.address); 

    // get our test signers
    const [deployer, address1, address2] = await hre.ethers.getSigners();

    // set mock event variables
    let deposit = hre.ethers.utils.parseEther("1");
    let maxCapcity = 3;
    let timestamp = 1718926200;
    let eventDataCID = "bafybeibhwfzx6oo5rymsxmkdxpmkfwyvbjrrwcl7cekmbzlupmp5ypkyfi";

    // create the event
    let txn = await rsvpContract.createNewEvent(deposit, maxCapcity, timestamp, eventDataCID);
    let wait = await txn.wait();
    console.log("NEW EVENT CREATED:", wait.events[0].event, wait.events[0].args);

    // get the event id
    let eventID = wait.events[0].args.eventID;
    console.log("EVENT ID:", eventID);

    // send deposit
    txn = await rsvpContract.createNewRSVP(eventID, {value: deposit});
    wait = await txn.wait();
    console.log("NEW RSVP:", wait.events[0].event, wait.events[0].args);

    // new rsvp w/ test account 1
    txn = await rsvpContract.connect(address1).createNewRSVP(eventID, {value: deposit});
    wait = await txn.wait();
    console.log("NEW RSVP:", wait.events[0].event, wait.events[0].args);

    // new rsvp w/ test account 2
    txn = await rsvpContract.connect(address2).createNewRSVP(eventID, {value: deposit});
    wait = await txn.wait();
    console.log("NEW RSVP:", wait.events[0].event, wait.events[0].args);

    // confirm all rsvps
    txn = await rsvpContract.confirmAllAttendees(eventID);
    wait = await txn.wait();
    wait.events.forEach((event) => console.log("CONFIRMED:", event.args.attendeeAddress));

    // test 7 day withdrawal window (wait 10 years)
    await hre.network.provider.send("evm_increaseTime", [15778800000000]);
    txn = await rsvpContract.withdrawUnclaimedDeposits(eventID);
    wait = await txn.wait();
    console.log("WITHDRAWN:", wait.events[0].event, wait.events[0].args);
};

const runMain = async () => {
    try {
        await main();
        process.exit(0);
    } catch (error) {
        process.exit(1);
    }
};

runMain();